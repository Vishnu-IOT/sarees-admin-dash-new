...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError("");
      try {
        const [productsRes, ordersRes, usersRes, categoriesRes] =
          await Promise.all([
      ...
      <>
        ...
        <div className="page">
          <div className="card card-pad" style={{ color: "var(--danger)" }}>
            Couldn't load the dashboard: {error}. Is the backend running at the
            configured API URL?
          </div>
        </div>
      </>
  ...
  const lowStock = products.filter((p) =>
    p.attributes?.some((variant) => Number(variant.quantity) <= 5),
  ).length;
      ...
      <div className="page">
        <div className="stat-grid">
          <StatCard
            ...
            note="From completed payments"
          />
          <StatCard
            icon={IconOrders}
            label="Total orders"
            value={ordersTotal}
            note={`${pendingOrders} pending action`}
          />
          <StatCard
            icon={IconBox}
            label="Products"
            value={productsTotal}
            note={`${lowStock} low on stock`}
          />
          <StatCard
            icon={IconUsers}
            label="Customers"
        ...
        <div className="dash-grid">
          ...
          <div className="card card-pad">
            ...
            {categoryCounts.length ? (
              ...
              <p className="muted" style={{ fontSize: 13 }}>
                Add a category to start organizing your catalogue.
              </p>
            )}
          </div>
        </div>
        ...
        {lowStock > 0 && (
          ...
          >
            <span className="stat-card-icon" style={{ color: "var(--danger)" }}>
              <IconClock />
            </span>
            <div style={{ flex: 1 }}>
              <strong>
            ...
            <Link to="/products?stock=low" className="btn btn-outline btn-sm">
              Review stock
            </Link>
          </div>
        )}
