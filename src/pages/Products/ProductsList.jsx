        ...
        <div className="table-wrap">
              ...
              <div className="collection-tabs">
                {[
                  ...
                  <button
                    key={tab.value}
                    type="button"
                    className={`collection-tab ${collection === tab.value ? "active" : ""
                      }`}
          ...
          {productsLoading ? (
            ...
            <EmptyState
              ...
              action={<button className="btn btn-gold btn-sm" onClick={openCreate}><IconPlus /> Add product</button>}
            />
          ) : (
                    ...
                    <tr key={p.id} className="clickable" onClick={() => navigate(`/products/${p.id}`)}>
                      <td>
                        <div className="row-thumb">
                          <ImageThumb src={p.attributes[0].image_url} alt={p.name} />
                          <div>
                            <div className="cell-primary">{p.name}</div>
                            <div className="cell-muted" style={{ fontSize: 12 }}>
                              {p.collection || '—'} {p.isNewArrival && <span className="badge badge-neutral" style={{ marginLeft: 6 }}>New</span>}
                              {p.loom && <span className="badge badge-gold" style={{ marginLeft: 6 }}>Loom</span>}
                            </div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="cell-primary">
                        ...
                        {p.discountPrice && (
                          <div className="cell-muted" style={{ fontSize: 11.5, textDecoration: 'line-through' }}>
                            {formatCurrency(p.price)}
                          </div>
                        )}
                      </td>
                      <td
                        className={p.totalQuantity <= 5 ? "cell-primary" : "cell-muted"}
                        style={
                          p.totalQuantity <= 5
                            ? { color: "var(--danger)" }
                            : undefined
                      ...
                      <td onClick={(e) => e.stopPropagation()}>
                        <div className="row-actions">
                          <button className="icon-btn" onClick={() => openEdit(p)} aria-label="Edit"><IconEdit /></button>
                          <button className="icon-btn danger" onClick={() => setDeleting(p)} aria-label="Delete"><IconTrash /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div style={{ padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border)' }}>
            <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>
              ...
              {" "}to{" "}
              {Math.min(pageProducts.currentPage * 10, pageProducts.total)}
              {" "}of{" "}
              {pageProducts.total} products
            </div>
            {pageProducts.totalPages > 1 && (
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <span style={{ padding: '6px 12px', fontSize: 13 }}>{pageProducts.currentPage} / {pageProducts.totalPages}</span>
                <button
                  className="btn btn-sm"
                  disabled={pageProducts.currentPage === pageProducts.totalPages}
                  onClick={() => {
