...
export default function CustomerDetail() {
  ...
  return (
    <>
      <Topbar eyebrow="Sales" title={`${user.data.name}`} />
      <div className="page">
        ...
        <div className="page-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div className="avatar-circle" style={{ width: 48, height: 48, fontSize: 16 }}>
              {initials(user.data.name, user.data.name)}
            </div>
            <div>
        ...
        <div className="stat-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
          ...
          <div className="stat-card">
            ...
            <div className="stat-card-value" style={{ fontSize: 20 }}>{formatDate(user.data.createdAt)}</div>
          </div>
        </div>
        ...
        <div className="detail-grid">
          ...
          <div className="card card-pad">
            ...
            <div className="kv-list">
              <div className="kv-row"><span className="k">Phone</span><span className="v">{user.phone || user.data.phone || '—'}</span></div>
              <div className="kv-row"><span className="k">Address</span><span className="v">{user.address || user.data.address || '—'}</span></div>
              <div className="kv-row"><span className="k">City</span><span className="v">{user.city || user.data.city || '—'}</span></div>
              <div className="kv-row"><span className="k">State</span><span className="v">{user.state || user.data.state || '—'}</span></div>
              <div className="kv-row"><span className="k">ZIP code</span><span className="v">{user.zipCode || user.data.zipCode || '—'}</span></div>
              <div className="kv-row"><span className="k">Country</span><span className="v">{user.country || user.data.country || '—'}</span></div>
            </div>
          </div>
        </div>
      </div>
